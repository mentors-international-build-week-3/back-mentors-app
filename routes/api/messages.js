...
router.post('/rsvpbot', async (req, res) => {
  ...
  Message.find({ phoneNumber: menteeNumber }, (err, message) => {
    ...
    if (message.length !== 0) {
      if (
        !message[0].menteeFirstName &&
        !message[0].menteeLastName &&
        !message[0].attending
      ) {
        Message.findByIdAndUpdate(
          message[0]._id,
          { $set: { menteeFirstName: smsBody } },
        ...
        Message.findByIdAndUpdate(
          ...
          () => {
            client.messages.create({
              ...
              body: `Awesome, ${
                message[0].menteeFirstName
              }! Last question: Do you plan on attending your next mentor session with us? Please reply "1" for YES or "0" (zero) for NO.`,
            });
          ...
          () => {
            if (smsBody === '1') {
              client.messages.create({
                ...
                body: `Perfect, ${
                  message[0].menteeFirstName
                }! We'll save a spot for you! Thanks for your time and have an awesome day!`,
              });
              ...
              client.messages.create({
                to: `${menteeNumber}`,
                from: `${appNumber}`,
                body: `Oh no! We're sorry to hear that you won't be joining us, ${
                  message[0].menteeFirstName
                }. Don't worry! We'll notify your mentor, ASAP, to reschedule another mentor session for you! :)`,
              });
              ...
              client.messages.create({
                ...
                body: `I'm sorry, ${
                  message[0].menteeFirstName
                }, that wasn't a valid response. Please try again, but try to reply "1" for YES or "0" (zero) for NO.`,
              });
          ...
          () => {
            if (smsBody === '1') {
              client.messages.create({
                ...
                body: `Perfect, ${
                  message[0].menteeFirstName
                }! We'll save a spot for you! Have an awesome day!`,
              });
              ...
              client.messages.create({
                ...
                body: `I'm sorry, ${
                  message[0].menteeFirstName
                }, you've exhaused the maximum number of attempts to answer this question. Please text "RSVP" to this number again to get access to more attempts! Thanks!`,
              });
      ...
      if (smsBody === 'RSVP' || smsBody === 'rsvp' || smsBody === 'Rsvp') {
        ...
        newMessage.save(() => {
          client.messages.create({
            to: `${menteeNumber}`,
            from: `${appNumber}`,
            body: `Hello! My name is RSVP BOT. I'm here to collect your RSVP. What is your first name?`,
          });
      ...
      message
        ...
        .catch(err => {
          res
            ...
            .json({
              delete_status:
                'Message was not deleted because this message could not be found',
            });
        });
    })
    .catch(err => {
      res
        ...
        .json({
          error_message:
            'Something went wrong while trying to delete this message from the database',
        });
    });
});
...
router.put('/:id', (req, res) => {
  Message.findById(req.params.id).then((message, err) => {
    if (!message) {
      res
        .status(404)
        .json({
          error_message:
            'Message was not updated because this message could not be found',
        });
    } else {
      req.body.menteeFirstName
        ? (message.menteeFirstName = req.body.menteeFirstName)
        : null;
      req.body.menteeLastName
        ? (message.menteeLastName = req.body.menteeLastName)
        : null;
      ...
      message
        ...
        .then(message => {
          res
            .status(202)
            .json({ update_status: 'Message was updated successfully' });
        })
        .catch(err => {
          res
            .status(400)
            .json({
              error_message:
                'Something went wrong while trying to update this message',
            });
        });
    }
  });
